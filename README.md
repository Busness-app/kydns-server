...
| `kydns service add\|list\|update\|rm` | Manage services, their addresses and aliases (`update` only changes `--mac`). `--mac` reserves the address for a device in the built-in DHCP server. |
